...
- Note: The frontend contains a production fallback to the canonical backend (https://hn3t.pythonanywhere.com) when VITE_API_BASE_URL is not provided at build time. This ensures the built artifact targets the intended backend, but requires the backend to allow CORS or for the deployment to proxy /api to the backend. To avoid CORS, set VITE_API_BASE_URL at build time to the explicit API origin or configure the server-side proxy.

Production environment contract:
- Set VITE_API_BASE_URL to the backend origin you want the built SPA to call (example: VITE_API_BASE_URL=https://hn3t.pythonanywhere.com).
- Alternatively, include a file at artifacts/workforce-console/.env.production with that variable before running pnpm --filter @workspace/workforce-console run build.
- The build will embed the canonical API origin into the compiled assets as `${VITE_API_BASE_URL}/api/v1` (or fall back to the hardcoded https://hn3t.pythonanywhere.com when not provided in non-dev builds). Ensure the backend permits CORS or that the deployed static server proxies /api to the backend origin.
